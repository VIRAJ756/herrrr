...
export async function getRiskAnalysis(input) {
    ...
    const avgSeverity = nearby.length > 0 ? nearby.reduce((sum, incident) => sum + incident.severity, 0) / nearby.length : 0;
    ...
    const result = {
        riskLevel,
        summary: nearby.length > 0
            ? `${nearby.length} recent incidents nearby. Highest concern signals: ${commonTypes}. Current zone risk is ${Math.round(blended * 100)}%.`
            : `No dense recent incident cluster detected. Current modeled area risk is ${Math.round(blended * 100)}% with ${zones.length > 0 ? "some nearby risk zone activity" : "limited recent activity"}.`,
...
export async function getSafeRoute(input) {
    const route = interpolateRoute(input.originLat, input.originLng, input.destLat, input.destLng, 6);
    const riskSegments = (await Promise.all(route.map(async (point) => {
...
function buildSafetyTips(riskLevel, incidentCount, timeOfDay) {
...
function interpolateRoute(startLat, startLng, endLat, endLng, steps) {
