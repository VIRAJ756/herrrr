...
export function invalidateRiskZoneCache() {
    riskCache = null;
}
/** Return zones within a given radius of a point. */
export async function getNearbyRiskZones(lat, lng, radiusMeters = ZONE_ALERT_RADIUS_METERS) {
    const zones = await getRiskZones();
    return zones.filter((zone) => haversineMeters(lat, lng, zone.latitude, zone.longitude) <= radiusMeters);
}
/** Build zones from incidents using a greedy geospatial grouping pass. */
export function buildRiskZones(incidents, now) {
    ...
    while (remaining.length > 0) {
        ...
        for (let i = remaining.length - 1; i >= 0; i -= 1) {
            ...
            if (haversineMeters(seed.latitude, seed.longitude, candidate.latitude, candidate.longitude) <=
...
function scoreCluster(cluster, now) {
    ...
    const severityAverage = severityWeightDenominator > 0 ? clamp(severityNumerator / severityWeightDenominator, 0, 1) : 0;
    const densityFactor = clamp(cluster.length / 5, 0, 1);
    const timeOfDayFactor = getTimeOfDayFactor(now);
    const score = density30d * 0.28 +
        density7d * 0.27 +
        severityAverage * 0.2 +
        densityFactor * 0.15 +
        timeOfDayFactor * 0.1;
...
function getTimeOfDayFactor(now) {
...
function centroid(cluster) {
    const totals = cluster.reduce((acc, incident) => {
